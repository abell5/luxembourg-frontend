...
function getStream() {
    ...
    $.ajax({
        method: "POST",
        dataType: 'text',
        url: "https://llm-viz.users.hsrn.nyu.edu/generate?init_prompt="+query+"&k=25&T=1.3&max_new_tokens=512&verbose=false&random_state="+random_state+"&sleep_time="+sleep_time,
        crossDomain: true,
        xhrFields: {
...
function barplot_new(data) {
    ...
    const height = 465 - margin.top - margin.bottom; // You can add 15 pixels of extra height here... not sure why, maybe marigns/padding?
    ...
    $(".tick").on( "click", function() {
        ...
        $.ajax({
            method: "POST",
            dataType: 'text',
            url: "https://llm-viz.users.hsrn.nyu.edu/regenerate?idx_counter="+idx_counter+"&new_token_str="+text+"&k=25&T=1.3&max_new_tokens=512&sleep_time="+sleep_time+"&verbose=true&random_state="+random_state,
            crossDomain: true,
            xhrFields: {
                onprogress: function (event) {
                    let chunk = event.currentTarget.responseText; //.responseText;
